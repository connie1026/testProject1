...
-(BOOL)ccPhysicsCollisionBegin:(CCPhysicsCollisionPair *)pair robotCollision:(CCNode *)robot bluediamondCollision:(CCNode *)bluediamond {
    _numOfCoins += 0;
    [bluediamond removeFromParentAndCleanup:YES];
    [(CCLabelTTF*)[self getChildByName:@"labelCoin" recursively:NO] setString:[NSString stringWithFormat:@"Coin: %d",_numOfCoins]];
...
-(BOOL)ccPhysicsCollisionBegin:(CCPhysicsCollisionPair *)pair robotCollision:(CCNode *)robot coinsCollision:(CCNode *)coins {
    _numOfCoins += 1;
...
-(BOOL)ccPhysicsCollisionBegin:(CCPhysicsCollisionPair *)pair superRobotCollision:(CCNode *)robot bluediamondCollision:(CCNode *)bluediamond {
    _numOfCoins += 0;
    [bluediamond removeFromParentAndCleanup:YES];
    [(CCLabelTTF*)[self getChildByName:@"labelCoin" recursively:NO] setString:[NSString stringWithFormat:@"Coin: %d",_numOfCoins]];
...
-(BOOL)ccPhysicsCollisionBegin:(CCPhysicsCollisionPair *)pair superRobotCollision:(CCNode *)robot coinsCollision:(CCNode *)coins {
    _numOfCoins += 1;
    [coins removeFromParentAndCleanup:YES];
    [(CCLabelTTF*)[self getChildByName:@"labelCoin" recursively:NO] setString:[NSString stringWithFormat:@"Coin: %d",_numOfCoins]];
...
-(void)generateOneObstacle : (CCSprite*)bg {
    ...
    int minY = bg.contentSize.height*.3;
    int maxY = bg.contentSize.height*.6 - spr.contentSize.height*.5;
    int randomY = arc4random()%(maxY-minY) + minY;
